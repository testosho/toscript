...
// ============================================================================
// SEQUENCE MODAL FUNCTIONS - ADD THESE TO YOUR SCRIPT.JS
// ============================================================================

// Function 1: Show Add Sequence Modal
function showAddSequenceModal() {
    console.log('showAddSequenceModal called');
    
    // Check if scenes exist
    if (!projectData.projectInfo.scenes || projectData.projectInfo.scenes.length === 0) {
        alert('No scenes found! Please write some scenes first.');
        return;
    }
    
    const modal = createSequenceModal();
    document.body.appendChild(modal);
    
    const input = modal.querySelector('#sequence-name-input');
    if (input) input.focus();
}

// Function 2: Create Sequence Modal HTML
function createSequenceModal() {
    console.log('createSequenceModal called');
    ...
    // Get current scene numbers for checkboxes
    const scenes = projectData.projectInfo.scenes || [];
    
    const sceneCheckboxes = scenes.map(scene => `
        <label style="display: flex; align-items: center; gap: 8px; padding: 8px; 
                      background: var(--background-color); border-radius: 4px; 
                      cursor: pointer; margin-bottom: 4px;">
            <input type="checkbox" class="sequence-scene-checkbox" value="${scene.number}" 
                   style="width: 18px; height: 18px; cursor: pointer;">
            <span style="font-family: 'Courier Prime', monospace; font-size: 13px;">
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 600px; max-height: 80vh;">
            <div class="modal-header">
                ...
                <button class="icon-btn close-modal-btn" style="font-size: 1.5rem;">&times;</button>
            </div>
            <div class="modal-body" style="overflow-y: auto;">
                <div class="form-group">
                    ...
                    <input type="text" id="sequence-name-input" 
                           placeholder="e.g., Opening Montage, Chase Scene, etc." 
                ...
                <div class="form-group" style="margin-top: 1.5rem;">
                    <label style="display: block; margin-bottom: 0.5rem; font-weight: 500;">
                        Select Scenes for this Sequence
                    </label>
                    <p style="font-size: 0.85rem; color: var(--muted-text-color); 
                              margin-bottom: 0.5rem;">
                        Check all scenes that belong to this sequence
                    </p>
                    <div style="max-height: 300px; overflow-y: auto; 
                                border: 1px solid var(--border-color); 
                                border-radius: 8px; padding: 10px; 
                                background: var(--surface-color);">
                        ${sceneCheckboxes || '<p style="color: var(--muted-text-color); text-align: center; padding: 20px;">No scenes available. Write some scenes first!</p>'}
                    </div>
                </div>
            </div>
            <div class="modal-footer" style="display: flex; gap: 10px; justify-content: flex-end; 
                ...
                <button class="main-action-btn secondary" id="cancel-sequence-btn">Cancel</button>
                <button class="main-action-btn" id="save-sequence-btn">Create Sequence</button>
            </div>
        </div>
    `;
    ...
    // Event listeners
    const closeBtn = modal.querySelector('.close-modal-btn');
    const cancelBtn = modal.querySelector('#cancel-sequence-btn');
    const saveBtn = modal.querySelector('#save-sequence-btn');
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', () => {
            const nameInput = modal.querySelector('#sequence-name-input');
            ...
            const checkboxes = modal.querySelectorAll('.sequence-scene-checkbox:checked');
            ...
            if (!projectData.projectInfo.sequences) {
                projectData.projectInfo.sequences = [];
            }
            
            const exists = projectData.projectInfo.sequences.find(s => s.name === name);
            if (exists) {
                alert('A sequence with this name already exists.');
                return;
            }
            
            projectData.projectInfo.sequences.push({
                name: name,
            ...
            if (typeof saveProjectData === 'function') {
                ...
                localStorage.setItem('toscriptData', JSON.stringify(projectData));
            }
            
            closeModal();
            alert(`Sequence "${name}" created with ${sceneNumbers.length} scene(s)!`);
...
// COMPLETE renderSceneList function - ADD THIS

function renderSceneList(scenes) {
    console.log('📝 renderSceneList called with', scenes?.length, 'scenes');
    
    const list = document.getElementById('scene-list');
    if (!list) {
        console.error('❌ scene-list not found');
        return;
    }
    ...
    if (!scenes || scenes.length === 0) {
        list.innerHTML = '<li style="padding:1rem;text-align:center;color:#9ca3af">No scenes</li>';
        return;
    }
    ...
    scenes.forEach((scene, idx) => {
        ...
        // Set innerHTML FIRST
        li.innerHTML = '<div class="scene-item-header"><span>' + scene.number + '. ' + (scene.sceneType || 'INT.') + '</span><span class="scene-time\">' + (scene.timeOfDay || 'DAY') + '</span></div><div style="font-size:0.85em;color:#9ca3af;margin-top:4px">' + (scene.location || scene.heading || 'Unknown') + '</div>';
        
        // Then set attribute AFTER innerHTML
        li.setAttribute('data-scene-number', String(scene.number));
        
        // Verify
        const check = li.getAttribute('data-scene-number');
        console.log(`  Scene ${scene.number} - data-scene-number = "${check}"`);
    ...
    // Final verification
    const withData = list.querySelectorAll('li[data-scene-number]');
    console.log(`✅ Rendered: ${scenes.length}, with data-scene-number: ${withData.length}`);
    
    if (withData.length === 0) {
        console.error('❌ CRITICAL: Attributes did not stick!');
    }
    ...
    if (typeof Sortable !== 'undefined') {
        ...
        list.sortableInstance = new Sortable(list, {
            ...
            onEnd: function(evt) {
                console.log('🎯 Dragged!');
                setTimeout(() => {
                    if (typeof reorderScript === 'function') reorderScript();
                }, 50);
            }
        });
    }
...
function updateFocusModeButton() {
	...
	// UPDATED: Properly reorder script in editor when scenes are dragged
	function reorderScript() {
	    console.log('Reordering script...');

	    const editor = document.getElementById('fountain-input');
	    const items = document.querySelectorAll('#scene-list li[data-scene-number]');

	    if (!editor || items.length === 0) {
	        console.error('Editor or scene list items not found.');
	        return;
	    }

	    // Get the new order from the list
	    const newOrder = Array.from(items).map(item => parseInt(item.dataset.sceneNumber));

	    // Ensure all scenes are visible (not filtered)
	    const totalScenes = projectData.projectInfo.scenes.length;
	    if (newOrder.length !== totalScenes && totalScenes > 0) {
	        alert('Error: Please set filter to "Show All Scenes" before reordering.');
	        renderSceneList(projectData.projectInfo.scenes);
	        return;
	    }

	    // Extract scene blocks from the editor text
	    const oldScript = editor.value;
	    const lines = oldScript.split('\n');
	    ...
	    for (let i = 0; i < lines.length; i++) {
	        const line = lines[i];
	        const isScene = /^(INT\.|EXT\.|INT\.\/EXT\.|EXT\.\/INT\.)/i.test(line.trim());

	        if (isScene) {
	            if (!pastHeader) {
	                headerLines = currentBlock;
	            } else {
	                blocks.push({ num: blockNum, lines: currentBlock });
	            }
	            pastHeader = true;
	            blockNum++;
	            currentBlock = [line];
	    ...
	    // Push the last scene block
	    if (pastHeader && currentBlock.length > 0) {
	        blocks.push({ num: blockNum, lines: currentBlock });
	    } else if (!pastHeader) {
	        headerLines = currentBlock;
	    }

	    // Reorder the blocks based on the new order
	    const reorderedBlocks = newOrder.map(num => {
	        const block = blocks.find(b => b.num === num);
	    ...
	    parts.push(...reorderedBlocks);

	    const newScript = parts.join('\n\n');

	    // Update the editor
	    editor.value = newScript;

	    // Save and re-sync the scene data
	    saveProjectData();
	    projectData.projectInfo.scenes = extractScenesFromText(newScript);

	    // Re-render the list with updated scene numbers
	    renderSceneList(projectData.projectInfo.scenes);

	    console.log('Script reordered and updated successfully.');
	}
	...
	// UPDATED: Export only currently visible/filtered scenes
	function exportSceneOrderAsCsv() {
	    console.log('=== EXPORTING SCENE ORDER ===');
    
	    // Get the currently displayed scenes from the scene list
	    const sceneListItems = document.querySelectorAll('#scene-list li[data-scene-number]');
    
	    if (sceneListItems.length === 0) {
	        alert('No scenes to export. Make sure scenes are visible in the navigator.');
	        return;
	    }
    
	    // Extract scene numbers from visible items
	    const visibleSceneNumbers = Array.from(sceneListItems).map(li => 
	        parseInt(li.dataset.sceneNumber)
	    );
    
	    // Get the full scene data for visible scenes only, in the current order
	    const scenesToExport = visibleSceneNumbers.map(num => 
	        projectData.projectInfo.scenes.find(scene => scene.number === num)
	    ).filter(scene => scene !== undefined);
    
	    if (scenesToExport.length === 0) {
	        alert('No scene data found to export.');
	        return;
	    }
	    ...
	    const rows = scenesToExport.map(scene => {
	        const sceneData = [
	            scene.number,
	            scene.sceneType || '',
	            scene.location || '',
	    ...
	    // Determine filter status for filename
	    const filterCategory = filterCategorySelect?.value || 'all';
	    let filterSuffix = '';
    
	    if (filterCategory !== 'all') {
	        if (filterCategory === 'sequence') {
	            const sequenceSelect = document.getElementById('filter-sequence-select');
	            const seqName = sequenceSelect?.value || 'filtered';
	            filterSuffix = `-${seqName.replace(/[^a-zA-Z0-9]/g, '_')}`;
	        } else {
	            filterSuffix = `-${filterCategory}`;
	        }
	    }
    
	    // Create filename
	    const filename = `${projectData.projectInfo.projectName || 'screenplay'}-scene-order${filterSuffix}.csv`;
    
	    // Download
	    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8' });
	    downloadBlob(blob, filename);
    
	    alert(`Scene order exported!\n\nFile: ${filename}\nScenes: ${scenesToExport.length}${filterSuffix ? '\nFilter: ' + filterCategory : ''}`);
	}
	...
	function handleFilterChange() {
	    ...
	    if (!category || category === 'all') {
	        ...
	        const sequenceSelect = document.getElementById('filter-sequence-select');
	        if (sequenceSelect) sequenceSelect.style.display = 'none';
	        ...
	        // SEQUENCE FILTER
	        console.log('Sequence filter selected');
	        console.log('Available sequences:', projectData.projectInfo.sequences);
        
	        // Hide text input
	        if (filterValueInput) filterValueInput.style.display = 'none';
        
	        // Create or show sequence dropdown
	        let sequenceSelect = document.getElementById('filter-sequence-select');
        
	        if (!sequenceSelect) {
	            sequenceSelect = document.createElement('select');
	            sequenceSelect.id = 'filter-sequence-select';
	            sequenceSelect.style.width = '100%';
	            sequenceSelect.style.padding = '0.5rem';
	            sequenceSelect.style.backgroundColor = 'var(--background-color)';
	            sequenceSelect.style.border = '1px solid var(--border-color)';
	            sequenceSelect.style.borderRadius = '6px';
	            sequenceSelect.style.color = 'var(--text-color)';
	            sequenceSelect.style.marginTop = '0.5rem';
	            sequenceSelect.style.fontSize = '16px';
            
	            // Insert after the category select
	            filterCategorySelect.parentElement.appendChild(sequenceSelect);
	        }
        
	        sequenceSelect.style.display = 'block';
        
	        // Get sequences (make sure array exists)
	        const sequences = projectData.projectInfo.sequences || [];
        
	        console.log('Populating dropdown with sequences:', sequences);
        
	        // Populate sequence dropdown
	        if (sequences.length === 0) {
	            sequenceSelect.innerHTML = '<option value="">No sequences created yet</option>';
	        } else {
	            sequenceSelect.innerHTML = '<option value="">-- Select Sequence --</option>' +
	                sequences.map(seq => 
	                    `<option value="${seq.name}">${seq.name} (${seq.sceneNumbers.length} scenes)</option>`
	                ).join('');
	        }
        
	        // Filter on change
	        sequenceSelect.onchange = function() {
	            const seqName = this.value;
	            console.log('Sequence selected:', seqName);
	            ...
	            if (sequence && sequence.sceneNumbers) {
	                const filtered = projectData.projectInfo.scenes.filter(scene => 
	                    sequence.sceneNumbers.includes(scene.number)
	                );
	        ...
	        if (filterHelpText) {
	            filterHelpText.textContent = sequences.length > 0 
	                ? 'Select a sequence to view its scenes' 
	                : 'No sequences created. Click + to create one.';
	            filterHelpText.style.display = 'block';
	        }
        
	    } else {
	        // TEXT FILTER (for other categories)
	        console.log('Text filter selected for category:', category);
        
	        // Hide sequence dropdown
	        const sequenceSelect = document.getElementById('filter-sequence-select');
	        if (sequenceSelect) sequenceSelect.style.display = 'none';
	        ...
	        if (filterHelpText) {
	            filterHelpText.style.display = 'block';
	            filterHelpText.textContent = 'Type to filter scenes';
	        }
	    }
	}
	...
	function applyFilter() {
	    ...
	    console.log('Applying filter - Category:', category, 'Value:', filterValue);
    
	    if (!category || category === 'all' || category === 'sequence') {
	        renderSceneList(projectData.projectInfo.scenes);
	        return;
	    }
	    ...
	    console.log('Total scenes to filter:', scenes.length);
    
	    switch(category) {
	        case 'sceneSetting':
	            filtered = scenes.filter(scene => {
	                const matchLocation = scene.location?.toLowerCase().includes(filterValue);
	                const matchHeading = scene.heading?.toLowerCase().includes(filterValue);
	                return matchLocation || matchHeading;
	            });
	            break;
            
	        case 'sceneType':
	            filtered = scenes.filter(scene => {
	                const match = scene.sceneType?.toLowerCase().includes(filterValue);
	                return match;
	            });
	            break;
	        ...
	        case 'cast':
	            filtered = scenes.filter(scene => {
	                ...
	                return scene.characters.some(char => 
	                    char.toLowerCase().includes(filterValue)
	                );
	            });
	            break;
	        ...
	        case 'timeOfDay':
	            filtered = scenes.filter(scene => {
	                const match = scene.timeOfDay?.toLowerCase().includes(filterValue);
	                return match;
	            });
	            break;
	...
	// Updated PDF Export Function - Prints Screenplay Preview Exactly
	async function saveScreenplayAsPDF() {
	    if (typeof window.jspdf === 'undefined' || typeof html2canvas === 'undefined') {
	    ...
	    if (!screenplayOutput || !screenplayOutput.innerHTML.trim()) {
	        alert('No screenplay content to export. Click "TO SCRIPT" first to preview your screenplay.');
	        return;
	    }

	    try {
	        // Show progress modal
	        showProgressModal('Generating PDF from screenplay preview...\nPlease wait...');

	        // Use html2pdf approach for exact visual rendering
	        const { jsPDF } = window.jspdf;
	        const doc = new jsPDF({
	            orientation: 'portrait',
	            unit: 'mm',
	        ...
	        // Get the screenplay output element
	        const element = screenplayOutput;
	        ...
	        // Render using html2canvas for accurate reproduction
	        ...
	        // Save the PDF
	        const filename = `${projectData.projectInfo.projectName || 'Untitled'}-screenplay.pdf`;
	        doc.save(filename);
        
	        hideProgressModal();
	        alert(`PDF created successfully!\n\nFile: ${filename}\n\nYour screenplay has been exported exactly as shown in the preview.`);
    ...
    function setupEventListeners() {
        ...
        ['scene-navigator-btn', 'scene-navigator-btn-script'].forEach(id => {
               ...
               if (btn) {
                   btn.addEventListener('click', e => {
                       ...
                       // 1. Extract fresh scene data first
                       ...
                       // 2. Call the correct render function
		...
		if (filterValueInput) {
		    filterValueInput.addEventListener('input', () => {
		        console.log('Filter input changed:', filterValueInput.value);
		        applyFilter();
		    });
